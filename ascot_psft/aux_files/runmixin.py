...
ONLY FOR REFERENCE, IN PRINCIPLE THIS ALL IS BUILT IN IN A5PY
...
class RunMixin(DistMixin):
    ...
    def getwall_figuresofmerit(self):
        ...
        pload : float
            Peak power load.
        """
        wetted, area, edepo, pdepo, iangle, _ = self.getwall_loads()
    ...
    def getwall_loads(self, weights=True, p_ids=None):
        ...
        vertices = self.wall.noderepresentation()[0]
        ...
        barycenters = np.mean(reshaped_array, axis=1)[wetted-1]        
        ...
        def cartesian_to_cylindrical(x, y, z):
            ...
            :return: Tuple containing (r, phi, z)
            """
            r = np.sqrt(x**2 + y**2)
            phi = np.arctan2(y, x)
            return r, phi, z
        barycenters_cylindrical = np.array([cartesian_to_cylindrical(x, y, z) for x, y, z in barycenters])
        ...
        return wetted, area, edepo, pdepo, iangle, barycenters_cylindrical
    ...
    def getwall_3dmesh(self, w_indices=None, p_ids=None):
        ...
        ids, area, eload, pload, iangle, barycenter = self.getwall_loads(p_ids=p_ids)
        # get the marker load at each tile
        _, _, _, mload, _, _ = self.getwall_loads(weights=False, p_ids=p_ids)
        ...
        area_tot = np.zeros((n_tri, )) + np.nan
        area_tot[ids] = area
        barycenter_tot = np.zeros((n_tri, 3)) + np.nan
        barycenter_tot[ids,:] = barycenter
        ...
        wallmesh.cell_data["area"] = area_tot[wall_f]
        wallmesh.cell_data["barycenter (r)"] = barycenter_tot[:,0][wall_f]
        wallmesh.cell_data["barycenters (phi)"] = barycenter_tot[:,1][wall_f]
        wallmesh.cell_data["barycenters (z)"] = barycenter_tot[:,2][wall_f]
        return wallmesh
    ...
    def plotwall_convergence(self, qnt, nmin=1000, nsample=10, axes=None):
        ...
        for i, n in enumerate(nsubset):
            ...
            if qnt == 'lostpower':
                ...
                ids0  = ids[idx]
                lost0 = lost[idx]
                _, area, loads, _, _, _ = self.getwall_loads(weights=True, p_ids=ids0[lost0])
    ...
    def plotwall_loadvsarea(self, axes=None):
        ...
        axes : :obj:`~matplotlib.axes.Axes`, optional
            The axes where figure is plotted or otherwise new figure is created.
        """
        _, area, eload, _, _, _ = self.getwall_loads()
